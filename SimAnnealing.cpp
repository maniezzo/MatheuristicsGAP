...
// This assumes to have in sol a solution to improve. implements multiplier djustment
int SimAnnealing::mathSimAnnealing(int** c, double k, double maxT, double alpha, int maxiter, int iterAnneal, double coefAnneal)
{
   ...
   vector<vector<int>> cPrime(m, vector<int>(n)); // cPrime costs wil be adjusted
...
start:
   ...
   for(i=0;i<m;i++)
      ...
      {  nInfeas++;
         for(j=0;j<n;j++)
            cPrime[i][j] -= capleft[i];  // cost adjustment
      }
